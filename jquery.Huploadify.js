...
$.fn.Huploadify = function(opts){
	var itemTemp = '<div id="${fileID}" class="uploadify-queue-item"><img src="" /><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div><span class="up_filename">${fileName}</span><span class="uploadbtn">上传</span><span class="delfilebtn">X</span></div>';
	var defaults = {
	...
	this.each(function(){
		...
		inputStr += '"/>';
		inputStr += '<div id="file_upload_'+instanceNumber+'-button"  class="uploadify-button">';
				  ...
				  xhr.onreadystatechange = function(e) {
					  if (xhr.readyState == 4) {
						  if (xhr.status == 200) {
							...
							thisfile.find(".uploadify-progress").hide();
						  } else {
